...
class ModelCheckpointManager:
    ...
    MODELS = {
        ...
        (ModelArchitecture.EQUIFORMER_V2, ModelVariant.LARGE): ModelInfo(
            ...
            registry_name="EquiformerV2-Large-S2EF-ODAC",
        ...
        (ModelArchitecture.GEMNET_OC, ModelVariant.LARGE): ModelInfo(
            ...
            registry_name="GemNet-OC-S2EF-ODAC",
        ...
        (ModelArchitecture.PAINN, ModelVariant.BASE): ModelInfo(
            name="PaiNN",
            registry_name="PaiNN-S2EF-ODAC",
            checkpoint_filename="PaiNN.pt",
    ...
    def get_checkpoint_path(self, architecture: ModelArchitecture, variant: ModelVariant = None) -> Path:
        ...
        if not model_info:
            raise ValueError(f"Unsupported model: {architecture.value} {variant.value if variant else ''}")
        return self.checkpoint_dir / model_info.checkpoint_filename
    
    def download_checkpoint(self, architecture: ModelArchitecture, variant: ModelVariant = None) -> Path:
        ...
        checkpoint_path = model_name_to_local_file(
            ...
            local_cache=str(self.checkpoint_dir)
        )
    ...
    def list_available_models(cls) -> Dict[tuple, str]:
        ...
        return {(info.architecture, info.variant): info.description 
